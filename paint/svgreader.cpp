...
void SvgReader::draw(QList<element> & figList, paintScene * scene){
    ...
    for(int i = 0; i < figList.size(); i++){
        ...
        if(mx.size() <= 5){ // если 5 элементов (1 угол - 2 угол -3 угол -4 угол - 1 угол), то либо квадрат либо ромб
            if(*std::min_element(begin(mx),end(mx)) == mx[1] || *std::min_element(begin(my), end(my)) == my[1]){
                QGraphicsRectItem * item = new QGraphicsRectItem();
                item->setRect(*std::min_element(begin(mx),end(mx)), *std::min_element(begin(my),end(my)), fabs(mx[0] - mx[1]), fabs(my[0] - my[3]) );
